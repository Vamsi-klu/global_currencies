...
export function renderCards(data, grid) {
    grid.innerHTML = '';
    if (data.length === 0) {
...
export function filterData(searchBox, grid) {
...
export function updateStats(currencyData) {
    ...
    const currencyCounts = getCurrencyCounts(currencyData);
...
export function getCurrencyCounts(currencyData) {
...
export function renderChart(currencyData, Chart) {
    const topCurrencies = getCurrencyCounts(currencyData).slice(0, 10).reverse();
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    searchBox.addEventListener('input', () => filterData(searchBox, grid));

    renderCards(currencyData, grid);
    updateStats(currencyData);
    // Chart is available globally via the script tag in the HTML
    // We need to check if Chart is defined before using it.
    if (typeof Chart !== 'undefined') {
        renderChart(currencyData, Chart);
    }
});
